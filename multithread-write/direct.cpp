...
void myWrite(int id, char dir[200], int block_size) {
    ...
    fd = open(filename, O_CREAT | O_RDWR | O_APPEND | O_DIRECT, S_IRUSR | S_IWUSR);
    if (fd == -1) {
        perror(filename);
        return;
    }
    ...
    while (true) {
        int ret = write(fd, (char*)(reinterpret_cast<uintptr_t>(block+4095)/4096*4096), block_size);
        if (ret < block_size-1) {
            time(&t);
...
int main(int argc, char *argv[]) {
    int num_thread = 48, block_size = 1'048'576;
    char directory[200] = ".";
    ...
    block = new char[block_size+8192];
    for (int i=0; i<block_size+8191; ++i) {
        block[i] = 'c';
    }
